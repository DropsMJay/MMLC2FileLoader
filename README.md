...
- **Optional debug logging** — two independently toggleable logs: one
  listing every resource the game requests (useful for discovering
  what's moddable), and one showing exactly which mods were picked up.
...
1. Enable `DEBUG_LOG_ALL_FILES` (see [Debug
   logging](#debug-logging-optional)), rebuild, and play through the
   area you want to mod. Check `mmlc2_all_resources_log.txt` for the
   exact resource path.
...
5. Launch the game with `DEBUG_LOG_MOD_REDIRECTS` enabled and confirm
   an `[OK]` line for your file in `mmlc2_mod_redirects_log.txt`.
...
|---|---|---|
| `DEBUG_LOG_ALL_FILES` | `mmlc2_all_resources_log.txt` | Deduplicated list of every resource requested this session — a live catalog of what can be modded. |
| `DEBUG_LOG_MOD_REDIRECTS` | `mmlc2_mod_redirects_log.txt` | Only the files actually replaced by a mod (`[OK]`), or mod files found but failed to read (`[FAIL]`). |

Neither log file is created if its flag is disabled. Flip the flags and
rebuild as needed.
...
Both are intercepted with inline hooks. When a matching file exists
under `mods/`, its contents are served in place of the original;
otherwise the original code path runs unmodified. Full details,
including every mapped function address, are in
